...
<div class:hidden={isHidden} class="z-40 overflow-auto scrolling-touch left-0 top-0 bottom-0 right-0 w-full h-full fixed bg-opacity-50 bg-black">
    <div class="relative mx-auto z-50 max-w-lg px-3 py-12">
        <div class="overflow-visible relative  rounded bg-white shadow">
            <button on:click={() => close()} class="flex items-center justify-center rounded-full absolute right-0 top-0 -m-3 h-8 w-8 transition duration-100 ease-in-out focus:ring-2 focus:ring-blue-500 focus:outline-none focus:ring-opacity-50 bg-gray-100 text-gray-600 hover:bg-gray-200">
